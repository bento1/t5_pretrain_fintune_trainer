...
#         rouge_dict = self.parse_score(rouge_results)    
    ...
    def on_train_epoch_end(self):
        ...
        avg_train_loss = torch.stack([x.view(1, -1) for x in self.train_step_outputs]).mean()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.validation_step_outputs.append(result.cpu())
        # print()
        return result
    ...
    def on_validation_epoch_end(self):
        ...
        avg_loss = torch.stack([x["val_loss"].view(1, -1) for x in self.validation_step_outputs]).mean()
    ...
    def optimizer_step(self, epoch, batch_idx, optimizer, optimizer_idx, second_order_closure=None, using_native_amp=False):
        # if self.trainer.use_tpu:#old no use_tpu
...
class Pretrain(Dataset):
    ...
    def split_into_segment(self, ds, input_length):
        ...
        for index, row in ds.iterrows():
            if len(new_rows)>self.num_samples:
                break
